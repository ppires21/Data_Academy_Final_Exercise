...
REPORT_PATH = os.path.join(REPORT_DIR, "quality_report.md")  # Output markdown report path
ENGINE = create_engine(build_db_url(cfg), future=True)        # DB engine for reading tables to validate
...
def run_quality_checks() -> None:
    ...
    ts = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%SZ")  # Current UTC timestamp for report header
    ...
    transacoes = pd.read_sql(f"SELECT * FROM {schema}.transacoes", ENGINE)       # Load transactions
    ...
    trx_errs = []
    trx_errs += _expect_not_null(transacoes, ["id", "id_cliente", "data_hora"])   # keys & timestamp not null
    # Simple parse check: if parsing yields NaT, it's invalid (vectorized)
    if pd.to_datetime(transacoes["data_hora"], errors="coerce").isna().any():     # Any invalid timestamp?
    ...
    item_errs = []
    item_errs += _expect_not_null(itens, ["id", "id_transacao", "id_produto", "quantidade", "preco_unitario"])  # not nulls
    item_errs += _expect_positive(itens, ["quantidade", "preco_unitario"])        # quantities and price > 0
    errors["transacao_itens"] = item_errs                                         # Record errors

    # Summarize results
    total_issues = sum(len(v) for v in errors.values())                            # Total count of issues across tables
    ...
    with open(REPORT_PATH, "w", encoding="utf-8") as f:                            # Open report file
        ...
        f.write("- Source: PostgreSQL normalized schema (Iteration 2 load)\n")     # Source lineage
        f.write(f"- Schema: `{schema}` → Validated tables: clientes, produtos, transacoes, transacao_itens\n")
        f.write("- Downstream targets: warehouse schema tables (Iteration 3)\n\n")  # Downstream lineage
    ...
    if total_issues > 0:                                                           # If there are any issues
        log.error(f"Data quality failed with {total_issues} issues. See {REPORT_PATH}")  # Log location
