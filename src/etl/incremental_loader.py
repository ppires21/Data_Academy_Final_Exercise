...
import os
import sys
sys.path.append(os.path.join(os.path.dirname(__file__), "../.."))  # ✅ Add project root early
...
from config.config_loader import get_config, build_db_url  # ✅ Now it can find this module
...
CHECKPOINT_FILE = os.path.join(CHECKPOINT_DIR, "transacoes_checkpoint.json")  # File for last processed time
...
def _fetch_increment(engine, since: datetime) -> pd.DataFrame:
    ...
    """)                                                      # Parameterized SQL
    df = pd.read_sql(q, engine, params={"window_start": window_start})  # Read into DataFrame
...
def _merge_into_dw(engine, df: pd.DataFrame) -> None:
    ...
    with engine.begin() as conn:                              # Transaction block
        conn.execute(text("CREATE SCHEMA IF NOT EXISTS warehouse"))                 # Ensure warehouse schema
        conn.execute(text("""
    ...
    df = df.sort_values(["id", "version_timestamp"]).drop_duplicates(subset=["id"], keep="last")
                                                              # Keep the newest version per id

    # Load increment to a temp table
    with engine.begin() as conn:                              # Transaction block for upsert
        df.head(0).to_sql("_tmp_increment", conn, schema="warehouse", if_exists="replace", index=False)  # Create temp
        df.to_sql("_tmp_increment", conn, schema="warehouse", if_exists="append", index=False)           # Fill temp
    ...
    log.info(f"Incremental upsert completed: {len(df)} rows processed.")  # Log processed count
...
def run():
    ...
    if not incr.empty:                                         # If any rows processed
        new_checkpoint = pd.to_datetime(incr["data_hora"]).max().to_pydatetime()  # Compute latest data_hora
        _save_checkpoint(new_checkpoint)                       # Save checkpoint forward
        log.info(f"Checkpoint advanced to: {new_checkpoint.isoformat()}")  # Log advancement
    else:
